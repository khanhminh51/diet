...
<?php
    if (isset($_POST['submit'])) {
        ...
        // 2. SQL to Check ID and Current password existed or not
        ...
        if ($res == TRUE) {
            ...
            if ($count == 1) {
                if ($new_password == $confirm_password) {
                    ...
                    if ($res2 == TRUE) {
                        $_SESSION['change_pass'] = "<h4 class='text-success text-center'> Password changed succesfully! </h4>";
                        echo("<script>location.href = '".SITEURL."user_page/user.php?id=$id';</script>");
                    } else {
                        $_SESSION['change_pass'] = "<h4 class='text-danger text-center'> Failed to change Password! </h4>";
                        echo("<script>location.href = '".SITEURL."user_page/user.php?id=$id';</script>");
                    }
                } else {
                    $_SESSION['pass_not_match'] = "<h6 class='text-danger'> New Password and Confirm Password NOT match ! </h6>";
                    echo("<script>location.href = '".SITEURL."user_page/change_password.php?id=$id';</script>");
                }
            } else {
